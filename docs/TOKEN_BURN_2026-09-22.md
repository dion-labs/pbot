...
## PB-012/031/038/041 local browser continuation — v0.1.5 candidate

Explicit authorization resumed clean at `88dcfae12ef91b64753398e7297d3d9508f68285`. The isolation plan is [BROWSER_ACCEPTANCE.md](BROWSER_ACCEPTANCE.md). Actual production dashboard bundle and HTTP API run with temporary Store/configuration, fictional records, fake device/controller classes installed before API import, forbidden API subprocess dispatch, fresh Chrome profile, page-request allowlist, local refusing HTTP(S) proxy and external DNS blocking. Trusted UI uses free3001 from the real Origin allowlist; API18731 and hostile origin5186. Owned listener PIDs are verified before transport use. No real adapter/account/runtime or other session was used.

Baseline `.../pbot-browser-acceptance-hy2f3xix` reproduced hidden command errors with unhandled rejection, Pause enabled during pending Stop, disconnected Run enabled,320/375px overflow to428px, and tooltip motion despite reduced-motion preference. Startup/empty/reconnect, scan/stop, start/stop confirmation, needs-attention/no-loop, wider layouts and keyboard baseline passed. Corrected security-only fixture `.../pbot-browser-acceptance-jqt01cfv` proves real hostile and opaque Origin403 plus owned rawHTTP Host400, unchangedStore digest and zero fake dispatch; the initial response-event assertion was a harness failure, not an API failure.

The first UI correction passed15/16cases (`.../pbot-browser-acceptance-n07gcdza`) but reproduced focus loss after disabling Pause. The targeted correction passed both keyboard and user-moved-focus checks (`.../pbot-browser-acceptance-b28b0ez3`). A real held HTTP snapshot then reproduced old ready state overwriting a later confirmed running state (`.../pbot-browser-acceptance-6lxp8osp`); a monotonically ordered load guard fixes that race.

Independent review reproduced two launcher ownership gaps. Four initial mocked negatives failed: stale parent relation adoption, missing identity treated as absent, uncaptured live direct handle, and empty global listing. Corrected fresh parent/child identity checks and explicit uncertainty handling pass eight ownership regressions; two archive source-selection checks also pass. Earlier cleanup receipts are superseded by the corrected runner. Root scoped review cleared the candidate and reservedv0.1.5.

Corrected final candidate: **17/17 actual Chrome cases passed**, no unhandled browser errors, no external page requests, forbidden-dispatch countzero, unchanged Store on hostile/opaque/Host rejection, and **verified-empty cleanup with no uncertain candidates**. Evidence `/var/folders/vq/rkb95_8n02d4c91kltpxr1r80000gp/T/pbot-browser-acceptance-3kutdi48`, log `/tmp/pbot-browser-final-candidate.log`. All abbreviated evidence paths above share that temporary parent. Browser version: 153.0.8010.48. Selected [before/after/error/focus screenshots](qa/2026-09-22/browser/README.md) are retained in this repository. Product hashes: dashboard `f186b42b581e6aae4121540cbc33788f0d7a9572ed4128fa064cfc885e9a5aea`; CSS `4e129aa896ca18e6488439c05d4b56860595430fcf5d98443ec5ae1e2ea26a08`.

Final source packaging now includes an explicit launcher hash in its report and archive-compatible source selection. Prepared metadata/download link forv0.1.5; priorv0.1.4 remains immutable. Next: validate exact committed archive (full build/render/Python/lint plus17browser cases), CI, publication and downloaded checksum. Unchanged13-case OS lifecycle acceptance fromv0.1.4 is reused; it is not being rerun to imply broader platform coverage.
